...
CLASS zcl_gcts_tr_analyzer DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! When TRUE, also report objects that depend on items outside the TR (Gap 7)
    ...
    "! Serialise analysis result as CSV for download / Excel (Gap 8)
    "! Columns: TR,RUN_TS,SRC_TASK,SRC_OBJ,TGT_TASK,TGT_OBJ,KIND,RISK,DETAIL,PULL_STEP,PULL_ACTION
    METHODS to_csv
      RETURNING VALUE(rv_csv) TYPE string.

    "! Persist result rows to ZGCTS_DEP_HISTORY (Gap 8)
    "! Safe to call multiple times — uses run timestamp as key, so each call creates a new snapshot.
  ...
  PRIVATE SECTION.
    ...
    " ── Gap 7: External dependency detection ─────────────────────────────────
...
" ═════════════════════════════════════════════════════════════════════════════
" CONSTRUCTOR — orchestrates all 4 stages
" ═════════════════════════════════════════════════════════════════════════════
  METHOD constructor.
    mv_tr = gv_tr_id.
    IF mv_tr IS INITIAL.
...
" ═════════════════════════════════════════════════════════════════════════════
" STAGE 1 — Task Inventory via XCO CTS API
" ═════════════════════════════════════════════════════════════════════════════
        ...
        LOOP AT lt_tasks INTO DATA(lo_task).
          ...
          TRY.
              LOOP AT lo_task->objects->all( ) INTO DATA(lo_obj).
                ...
                " obj_type stored as "R3TR/CLAS" — keep full key for context
                APPEND VALUE #(
                  task_id  = lv_task_id
                  obj_type = ls_key-pgmid && '/' && ls_key-object
                  obj_name = ls_key-obj_name ) TO mt_objects.
              ENDLOOP.
          CATCH cx_root.  " task objects unreadable — skip silently
          ENDTRY.
        ENDLOOP.
...
" ═════════════════════════════════════════════════════════════════════════════
  METHOD stage2_dependencies.
    LOOP AT mt_objects INTO DATA(ls_obj).
      " Extract rightmost segment of "R3TR/CLAS" → "CLAS"
...
" ─────────────────────────────────────────────────────────────────────────────
  METHOD deps_for_clas.
    TRY.
        ...
        TRY.  " interfaces (IMPLEMENTS — HIGH)
  ...
  METHOD deps_for_ddls.
    TRY.
        LOOP AT xco_cp_cds=>view_entity( iv_name
          ...
          DATA(lv_tgt_task) = task_of_object( lv_src ).
...
" ─────────────────────────────────────────────────────────────────────────────
  METHOD deps_for_ddlx.
    " Metadata extension depends on its base CDS view (DDLS)
    TRY.
...
" ─────────────────────────────────────────────────────────────────────────────
  METHOD deps_for_bdef.
    " RAP behavior definition depends on its root CDS view (DDLS)
    TRY.
...
" ─────────────────────────────────────────────────────────────────────────────
  METHOD deps_for_fugr.
    " Function group: extract function modules and their called groups
    TRY.
        DATA(lo_fg) = xco_cp_abap=>function_group( iv_name ).
        LOOP AT lo_fg->function_modules->all( ) INTO DATA(lo_fm).
          TRY.
              " Each function module may USE types from other objects in TR
...
" ═════════════════════════════════════════════════════════════════════════════
" STAGE 2b — Same-Object Conflict Detection (CRITICAL severity)
" Two tasks owning the same object = silent overwrite risk on import
" ═════════════════════════════════════════════════════════════════════════════
  METHOD stage2b_conflicts.
    " For each object, collect all tasks that own it
    TYPES: BEGIN OF ty_obj_tasks,
             obj_name TYPE string,
             tasks    TYPE string,   " comma-separated
           END OF ty_obj_tasks.
    ...
    LOOP AT mt_objects INTO DATA(ls_o).
      ...
      ELSE.
        " Only append if this task not already listed
    ...
    " Any object owned by more than one task is a CONFLICT
    LOOP AT lt_map INTO DATA(ls_map).
      IF ls_map-tasks CA ','.
        " Extract individual tasks from comma-separated string
        DATA(lt_task_list) = lcl_string_util=>split( iv_str = ls_map-tasks iv_sep = ',' ).
        ...
        LOOP AT lt_task_list INTO DATA(lv_task).
          ...
          ELSE.
            APPEND VALUE #(
              ...
              kind          = 'CONFLICT'
              detail        = |{ ls_map-obj_name } owned by both { lv_first } and { lv_task }| )
              TO mt_deps.
          ENDIF.
        ENDLOOP.
      ENDIF.
...
" ═════════════════════════════════════════════════════════════════════════════
" STAGE 3 — Cluster Detection (Union-Find with path compression)
" ═════════════════════════════════════════════════════════════════════════════
  METHOD stage3_clusters.
    DATA lt_uf TYPE tt_uf.

    " Seed: each unique task is its own root
    LOOP AT mt_objects INTO DATA(ls_o).
    ...
    " Assign risk: CRITICAL > HIGH > MEDIUM > NONE
  ...
  METHOD uf_find.
    TRY.
        ...
        ELSE.
          ...
          ct_uf[ task = iv_task ]-parent = rv_root.  " path compression
        ENDIF.
    CATCH cx_sy_itab_line_not_found.
      rv_root = iv_task.  " task not in UF — treat as own root
    ENDTRY.
  ENDMETHOD.
...
" ═════════════════════════════════════════════════════════════════════════════
" STAGE 4 — Pull-Order Console Output (F9 / standalone mode)
" ═════════════════════════════════════════════════════════════════════════════
  METHOD stage4_output.
    DATA lt_unique_tasks TYPE SORTED TABLE OF string WITH UNIQUE KEY table_line.
    LOOP AT mt_objects INTO DATA(ls_o). INSERT ls_o-task_id INTO TABLE lt_unique_tasks. ENDLOOP.

    out_header( ).
    out( |  TR: { mv_tr } | &&
         |  Tasks: { lines( lt_unique_tasks ) } | &&
         |  Objects: { lines( mt_objects ) } | &&
         |  Edges: { lines( mt_deps ) }| ).
    out( '' ).
    ...
    LOOP AT mt_clusters INTO DATA(ls_step).
      DATA(lv_action) = SWITCH #( ls_step-risk
        WHEN c_risk_critical THEN 'COORDINATE first, then pull TOGETHER'
        WHEN c_risk_high     THEN 'Pull TOGETHER'
        WHEN c_risk_medium   THEN 'Pull together (recommended)'
        WHEN c_risk_none     THEN 'Pull alone' ).
      out( |  Step { lv_step }: { lv_action } → { ls_step-tasks }| ).
      lv_step += 1.
    ENDLOOP.
    out( '-----------------------------------------------------------------' ).
  ENDMETHOD.
...
" ═════════════════════════════════════════════════════════════════════════════
" TO_JSON — serialises full result for ICF HTTP response
" ═════════════════════════════════════════════════════════════════════════════
  METHOD to_json.
    DATA lt_unique_tasks TYPE SORTED TABLE OF string WITH UNIQUE KEY table_line.
    LOOP AT mt_objects INTO DATA(ls_o). INSERT ls_o-task_id INTO TABLE lt_unique_tasks. ENDLOOP.
    ...
    LOOP AT mt_clusters INTO DATA(ls_cl).
      ...
      SPLIT ls_cl-tasks AT ',' INTO TABLE DATA(lt_tasks).
      LOOP AT lt_tasks INTO DATA(lv_t).
        IF lv_tasks_arr IS NOT INITIAL. lv_tasks_arr &&= ','. ENDIF.
        lv_tasks_arr &&= |"{ json_escape( condense( lv_t ) ) }"|.
      ENDLOOP.
      ...
      LOOP AT mt_deps INTO DATA(ls_dep).
        CHECK ls_cl-tasks CS ls_dep-source_task.
        IF lv_edges_arr IS NOT INITIAL. lv_edges_arr &&= ','. ENDIF.
        lv_edges_arr &&= `{` &&
      ...
      IF lv_clusters IS NOT INITIAL. lv_clusters &&= ','. ENDIF.
      lv_clusters &&= `{` &&
        |"risk":"{ ls_cl-risk }",| &&
        |"tasks":[{ lv_tasks_arr }],| &&
        |"edges":[{ lv_edges_arr }]| &&
    ...
    LOOP AT mt_clusters INTO DATA(ls_step).
      ...
      SPLIT ls_step-tasks AT ',' INTO TABLE DATA(lt_st).
      LOOP AT lt_st INTO DATA(lv_st).
        IF lv_step_tasks IS NOT INITIAL. lv_step_tasks &&= ','. ENDIF.
        lv_step_tasks &&= |"{ json_escape( condense( lv_st ) ) }"|.
      ENDLOOP.
      DATA(lv_action) = SWITCH #( ls_step-risk
        WHEN c_risk_critical THEN 'COORDINATE'
        WHEN c_risk_high     THEN 'TOGETHER'
        WHEN c_risk_medium   THEN 'TOGETHER_RECOMMENDED'
        WHEN c_risk_none     THEN 'ALONE' ).
      IF lv_pull_order IS NOT INITIAL. lv_pull_order &&= ','. ENDIF.
      lv_pull_order &&= `{"step":` && lv_step &&
                        `,"action":"` && lv_action && `"` &&
                        `,"tasks":[` && lv_step_tasks && `]}`.
      lv_step += 1.
    ENDLOOP.
...
" ═════════════════════════════════════════════════════════════════════════════
  METHOD task_of_object.
    " Returns the task ID that owns this object name, or '' if outside the TR
    TRY.
  ...
  METHOD add_dep.
    " Only record cross-task edges (same-task or external → skip)
...
" ═════════════════════════════════════════════════════════════════════════════
" GAP 7 — External dependency recording (INFO severity)
" Called from stage2_dependencies when the target object is outside the TR.
" Only fires when gv_include_external = abap_true.
" ═════════════════════════════════════════════════════════════════════════════
  METHOD add_external_dep.
    IF gv_include_external = abap_false. RETURN. ENDIF.
    IF iv_src_task IS INITIAL. RETURN. ENDIF.
    " External edges use '' as target task (outside TR) and kind prefixed 'EXT_'
    APPEND VALUE #(
      source_task   = iv_src_task
      source_object = iv_src_obj
      target_task   = ''
...
" ═════════════════════════════════════════════════════════════════════════════
" GAP 8 — Pull-step helpers (used by to_csv / persist_result)
" ═════════════════════════════════════════════════════════════════════════════
...
" ═════════════════════════════════════════════════════════════════════════════
" GAP 8 — to_csv: flat CSV of all dependency edges
" Header: TR,RUN_TS,SRC_TASK,SRC_OBJ,TGT_TASK,TGT_OBJ,KIND,RISK,DETAIL,PULL_STEP,PULL_ACTION
" ═════════════════════════════════════════════════════════════════════════════
  METHOD to_csv.
    ...
    LOOP AT mt_deps INTO DATA(ls_dep).
      DATA(lv_risk) = VALUE #( mt_clusters[
        KEY primary_key WHERE tasks CS ls_dep-source_task ] DEFAULT VALUE #( risk = '' ) )-risk.

      DATA(lv_step)   = pull_step_of_task(   ls_dep-source_task ).
      DATA(lv_action) = pull_action_of_task( ls_dep-source_task ).

      rv_csv &&= |{ csv_esc( mv_tr ) },| &&
                 |{ csv_esc( lv_ts ) },| &&
...
" ═════════════════════════════════════════════════════════════════════════════
" GAP 8 — persist_result: write edges to ZGCTS_DEP_HISTORY
" ═════════════════════════════════════════════════════════════════════════════
